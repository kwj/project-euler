...
module Make (Ord: OrderedType) : (S with type elt = Ord.t) = struct
  ...
  let size pq =
    if Array.length pq.data = 0 then 0 else pq.size

  let is_empty pq =
    if size pq = 0 then true else false
  ...
  let rec up_heap pq idx =
    ...
    if idx > 0 then (
      ...
      if (Ord.compare pq.data.(idx) pq.data.(p_idx)) > 0 then (
        swap pq idx p_idx;
        up_heap pq p_idx
      )
    )
  ...
  let rec down_heap pq idx =
    ...
    let lc_idx = lchild idx and rc_idx = rchild idx and tgt_idx = ref idx in
    if lc_idx < pq.size && (Ord.compare pq.data.(lc_idx) pq.data.(!tgt_idx)) > 0 then (
      tgt_idx := lc_idx
    );
    if rc_idx < pq.size && (Ord.compare pq.data.(rc_idx) pq.data.(!tgt_idx)) > 0 then (
      tgt_idx := rc_idx
    );
    if idx <> !tgt_idx then (
      swap pq idx !tgt_idx;
  ...
  let insert pq x =
    if Array.length pq.data = 0 then (
      pq.data <- Array.make pq.size x;
      pq.size <- 1
    ) else (
      if Array.length pq.data = pq.size then (
        expand pq
      );
      pq.data.(pq.size) <- x;
      pq.size <- pq.size + 1;
  ...
  let extract pq =
    ...
    | n when n = 1 -> pq.size <- 0; pq.data.(0)
    | n -> let result = pq.data.(0) in
