...
let solve limit =
  ...
  let rec dfs l_numer l_denom r_numer r_denom =
    let n = l_numer + r_numer and
        m = l_denom + r_denom in
    if m > upper_m then
      ()
    else (
      if (m + n) mod 2 = 1 then (
      ...
      dfs l_numer l_denom n m;
      dfs n m r_numer r_denom
    )
  in
  dfs 0 1 1 1;
