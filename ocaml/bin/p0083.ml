...
let make_neighbor_tbl x_size y_size =
  ...
  for x = 0 to x_size - 1 do
    for y = 0 to y_size - 1 do
      tbl.(x).(y) <- ([(x-1, y); (x+1, y); (x, y-1); (x, y+1)]
...
let solve arr_lst =
  let x_size = Array.length arr_lst and
      y_size = Array.length arr_lst.(0) in
  let nbr_tbl = make_neighbor_tbl x_size y_size and
      dist_tbl = make_distance_tbl x_size y_size in
  ...
  let rec loop () =
    if PQ.is_empty pq then
      dist_tbl.(x_size - 1).(y_size - 1)
    else
      let d, (i, j) = PQ.extract pq in
      let rec aux = function
        | [] -> ()
        | (x, y) :: xs -> let new_d = d + arr_lst.(x).(y) in
                          match new_d < dist_tbl.(x).(y) with
                          | true -> dist_tbl.(x).(y) <- new_d;
