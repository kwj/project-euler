...
fn compute(exp: u32) -> u64 {
    ...
    let pow_tbl: Vec<u64> = (0_u64..=9).map(|x| x.pow(exp)).collect();
    let mut acc = 0_u64;
    ...
    for n_digits in 2..=get_max_ndigits(exp) {
        for tpl in (0_u64..=9).combinations_with_replacement(n_digits) {
            let tmp: u64 = tpl.iter().map(|x| pow_tbl[*x as usize]).sum();
