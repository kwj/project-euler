...
class Sieve:
    ...
    def __init__(self, begin: int, end: int):
        ...
        self._mini_primes: list[int] = []
        self._mini_tbl: list[int] = []
        if end != 0:
            self._update_minitbl()
            self._make_prime_tbl()

    def _update_minitbl(self, new_size: int = 0):
        def start_pos(n: int) -> int:
        ...
        for i in range(3, limit, 2):
            if self._mini_tbl[i] != 0:
                self._mini_tbl[start_pos(i) :: i] = [0] * len(
                    self._mini_tbl[start_pos(i) :: i]
                )
    ...
    def _make_prime_tbl(self):
        ...
        for i in self._mini_primes:
            if i * i > self._end:
                break
            self._prime_tbl[start_pos(i) :: i] = [0] * len(
                self._prime_tbl[start_pos(i) :: i]
            )
    ...
    def update(self, begin: int, end: int) -> Self:
        if self._end == 0:
