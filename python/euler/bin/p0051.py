...
def is_probable(i, pat, current_min):
...
def find_prime(i, pat):
    ...
    for r in range(start, 10):
        if is_prime(tmp := assemble_num(i, pat, r)) == True:
            lst.append(tmp)
    ...
    for ndigits in count(4):
        ...
        for n in range(3, ndigits, 3):
            ...
            for i in range(10 ** (ndigits - n - 1) + 1, 10 ** (ndigits - n), 2):
                ...
                for pat in pat_lst:
                    if is_probable(i, pat, answer) == True:
