...
Dir = Enum('Dir', ['RIGHT', 'DOWN', 'UPPER_RIGHT', 'DOWN_RIGHT'])

QTY = 4
ROW_SIZE = len(grid_arr)
COL_SIZE = len(grid_arr[0])


def product(row: int, col: int, dir: Dir) -> int:
    v = 1
    for i in range(QTY):
        match dir:
            case Dir.RIGHT:
                v *= grid_arr[row][col + i]
            case Dir.DOWN:
                v *= grid_arr[row + i][col]
            case Dir.UPPER_RIGHT:
    ...
    for row in range(ROW_SIZE):
        for col in range(COL_SIZE):
            ...
            if col <= COL_SIZE - QTY:
                ...
                if row >= QTY - 1:
                    result = max(result, product(row, col, Dir.UPPER_RIGHT))
                if row <= ROW_SIZE - QTY:
                    result = max(result, product(row, col, Dir.DOWN_RIGHT))
