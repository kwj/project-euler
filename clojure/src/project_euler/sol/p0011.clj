...
(defn- find-max-product
  [matrix len]
  (let [rows (count matrix)
        cols (count (get matrix 0))
        val-at (fn [r c]
                 ;; If the position is out of the matrix, return 0.
                 ;; Because this problem requires the maximum *product* of four adjacent numbers.
                 (if (and (<= 0 r (dec rows)) (<= 0 c (dec cols)))
                   (get-in matrix [r c])
                   0))
        vecs-of-lsts (for [r (range rows)
                           c (range cols)]
    ...
    (transduce xf max Long/MIN_VALUE vecs-of-lsts)))
