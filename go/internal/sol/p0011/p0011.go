	...
	for r := range nRow {
		for c := range nCol {
			...
			if c <= nCol-length {
				tmp := make([]int, 0)
				for i := range length {
					tmp = append(tmp, matrix[r][c+i])
				}
				answer = append(answer, prodInts(tmp...))
			}
			...
			if r <= nRow-length {
				tmp := make([]int, 0)
				for i := range length {
					tmp = append(tmp, matrix[r+i][c])
				}
				answer = append(answer, prodInts(tmp...))
			}

			// upper right
			if r >= length-1 && c <= nCol-length {
				tmp := make([]int, 0)
				for i := range length {
					tmp = append(tmp, matrix[r-i][c+i])
				}
				answer = append(answer, prodInts(tmp...))
			}

			// down right
			if r <= nRow-length && c <= nCol-length {
				tmp := make([]int, 0)
				for i := range length {
					tmp = append(tmp, matrix[r+i][c+i])
				}
